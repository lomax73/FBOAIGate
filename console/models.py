...
class SessioneTerminale(models.Model):
    target = models.ForeignKey(Target, on_delete=models.CASCADE, related_name='sessioni_terminale')
    ...
    def __str__(self):
        return f'{self.utente} su {self.target.nome} — {self.aperta_il:%Y-%m-%d %H:%M}'
